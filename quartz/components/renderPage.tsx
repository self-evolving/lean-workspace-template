...
import PreviewDeploymentBannerConstructor, {
  hasPreviewDeploymentBanner,
} from "./PreviewDeploymentBanner"
import ChangedPageBannerConstructor, { changedPageForFileData } from "./ChangedPageBanner"
...
export function renderPage(
  ...
  const PreviewDeploymentBanner = PreviewDeploymentBannerConstructor()
  const ChangedPageBanner = ChangedPageBannerConstructor()
  const frame = resolveFrame(frameName)
  const changedPage = changedPageForFileData(componentData.fileData)
  const bodyClass =
    [
      hasPreviewDeploymentBanner() ? "has-sepo-preview-banner" : undefined,
      changedPage ? "has-sepo-pr-changed-page" : undefined,
    ]
      .filter(Boolean)
      .join(" ") || undefined
  ...
  const doc = (
    <html lang={lang} dir={direction}>
      ...
      <body
        data-slug={slug}
        class={bodyClass}
        data-sepo-pr-changed-page={changedPage ? "true" : undefined}
        data-sepo-pr-changed-source={changedPage?.sourcePath}
      >
        <PreviewDeploymentBanner {...componentData} />
        <ChangedPageBanner {...componentData} changedPage={changedPage} />
