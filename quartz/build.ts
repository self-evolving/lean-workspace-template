...
async function buildQuartz(argv: Argv, mut: Mutex, clientRefresh: () => void) {
  ...
  // blueprint-as-source patch: literate .lean chapters go through the markdown pipeline
  const markdownPaths = allFiles.filter((fp) => fp.endsWith(".md") || fp.endsWith(".lean")).sort()
  console.log(
  ...
  // blueprint-as-source patch: .lean pages slug like .md pages (shared normalization)
  ctx.allSlugs = allFiles.map((fp) => slugifyFilePath(leanAwareSlugPath(fp) as FilePath))
