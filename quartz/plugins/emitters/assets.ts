...
const filesToCopy = async (argv: Argv, cfg: QuartzConfig, excludeExtensions: Set<string>) => {
  // blueprint-as-source patch: .lean chapters are pages, not raw assets
  const excludePatterns = ["**/*.md", "**/*.lean", ...cfg.configuration.ignorePatterns]
  ...
  return {
    ...
    async *partialEmit(ctx, _content, _resources, changeEvents) {
      ...
      for (const changeEvent of changeEvents) {
        ...
        if (ext === ".md" || ext === ".lean" || excludeExtensions.has(ext)) continue
