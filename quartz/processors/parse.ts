...
// blueprint-as-source patch: lean chapter weave (dependency-light module)
// @ts-ignore -- plain mjs, no type declarations
import { weaveWithFrontmatter, leanAwareSlugPath } from "../../scripts/lib/lean-weave.mjs"
  ...
  return async (processor: QuartzMdProcessor) => {
    ...
    for (const fp of fps) {
      try {
        ...
        // blueprint-as-source patch: literate .lean chapters are markdown-like —
        // weave doc blocks + code into markdown before the normal pipeline.
        if (fp.endsWith(".lean")) {
          file.value = weaveWithFrontmatter(file.value.toString())
        }
        ...
        // blueprint-as-source patch: .lean pages slug like .md pages (shared
        // normalization — keep in sync with build.ts via leanAwareSlugPath)
        file.data.slug = slugifyFilePath(leanAwareSlugPath(file.data.relativePath) as FilePath)
